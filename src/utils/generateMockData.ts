...
const generateMenuItems = (category: ShopCategory): MenuItem[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const imageIndex = i + (foodItem.charCodeAt(0) % 5); // Create variation in image indices
    ...
    items.push({
      ...
      image: `https://source.unsplash.com/featured/?indian,${foodItem},food&${imageIndex}`, // More unique parameter
...
// Updated Indian-themed images for shops
...
const generateShop = (category: ShopCategory, index: number): Shop => {
  ...
  const indianCities = ['Mumbai', 'Delhi', 'Bangalore', 'Hyderabad', 'Chennai', 'Kolkata', 'Pune', 'Jaipur', 'Ahmedabad', 'Lucknow'];
  const indianCity = indianCities[index % indianCities.length]; // Ensure city variation
  
  // Indian shop names based on category
  const getIndianShopName = () => {
    const prefixes = {
      restaurant: ['Spice', 'Taj', 'Royal', 'Curry', 'Tandoor', 'Masala', 'Delhi', 'Mumbai', 'Bombay', 'Chennai'],
      retail: ['Bazar', 'Emporium', 'Mart', 'Dukaan', 'Silk', 'Fabindia', 'Mysore', 'Rajasthani', 'Swadeshi', 'Khadi'],
      service: ['Seva', 'Quick', 'Express', 'Pro', 'Perfect', 'Bharath', 'Indian', 'Metro', 'City', 'Aadhar'],
      streetFood: ['Chaat', 'Dhaba', 'Redi', 'Wala', 'Stall', 'Delhi', 'Bombay', 'Punjabi', 'Desi', 'Masti']
    };
    
    const suffixes = {
      restaurant: ['Palace', 'Kitchen', 'Darbar', 'Garden', 'House', 'Pavilion', 'Express', 'Delight', 'Rasoi', 'Bhavan'],
      retail: ['Stores', 'Center', 'Textiles', 'Traders', 'Shop', 'Bazaar', 'Market', 'Gallery', 'Collections', 'Heritage'],
      service: ['Services', 'Solutions', 'Care', 'Experts', 'Pros', 'Associates', 'Hub', 'Network', 'Point', 'Connect'],
      streetFood: ['Corner', 'Junction', 'Express', 'Point', 'Spot', 'Gully', 'Square', 'Lane', 'Street', 'Hub']
    };
    
    const prefix = prefixes[category][index % prefixes[category].length];
    const suffix = suffixes[category][index % suffixes[category].length];
  ...
  return {
    ...
    category,
    image: indianShopImages[category][index % indianShopImages[category].length],
